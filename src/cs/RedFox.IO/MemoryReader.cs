...
{
    /// <summary>
    /// Provides a simple method to read continuous data from a <see cref="ReadOnlyMemory{T}"/> byte buffer.
    /// <para>
    /// Unlike <see cref="SpanReader"/>, this type is a regular struct (not a ref struct),
    /// so it can be stored as a field in classes, used in async methods, and passed freely.
    /// It can produce both <see cref="ReadOnlyMemory{T}"/> slices (for zero-copy storage)
    /// and <see cref="ReadOnlySpan{T}"/> slices (for hot-path reads).
    /// </para>
    /// </summary>
    /// <param name="buffer">The buffer to read from.</param>
    public struct MemoryReader(ReadOnlyMemory<byte> buffer)
    {
        ...
        /// <summary>
        /// Gets a <see cref="ReadOnlySpan{T}"/> over the entire underlying buffer.
        /// </summary>
        public readonly ReadOnlySpan<byte> Span => _buffer.Span;

        /// <summary>
        /// Gets the entire underlying <see cref="ReadOnlyMemory{T}"/> buffer.
        /// </summary>
        public readonly ReadOnlyMemory<byte> Memory => _buffer;
        ...
        {
            var result = _buffer.Span.Slice(_position, size);
            _position += size;
            return result;
        }
        ...
        /// <para>
        /// Unlike <see cref="Read(int)"/> which returns a <see cref="ReadOnlySpan{T}"/>,
        /// the returned memory can be stored as a field, passed to constructors, and used
        /// for zero-copy slicing of the underlying buffer.
        /// </para>
        /// </summary>
        /// <param name="size">The number of bytes to read.</param>
        /// <returns>A <see cref="ReadOnlyMemory{T}"/> over the requested region of the buffer.</returns>
        ...
        /// <summary>
        /// Reads a <see cref="ReadOnlyMemory{T}"/> slice of the given size from the specified position.
        /// This does not modify the current position.
        /// <para>
        /// Unlike <see cref="Read(int, int)"/> which returns a <see cref="ReadOnlySpan{T}"/>,
        /// the returned memory can be stored as a field, passed to constructors, and used
        /// for zero-copy slicing of the underlying buffer.
        /// </para>
        /// </summary>
        ...
        /// <param name="size">The number of bytes to read.</param>
        /// <returns>A <see cref="ReadOnlyMemory{T}"/> over the requested region of the buffer.</returns>
        public readonly ReadOnlyMemory<byte> ReadMemory(int position, int size) => _buffer.Slice(position, size);

        /// <summary>
        /// Returns a <see cref="ReadOnlyMemory{T}"/> slice from the current position to the end of the buffer,
        /// without advancing the position.
        /// </summary>
        /// <returns>A <see cref="ReadOnlyMemory{T}"/> from the current position to the end.</returns>
        public readonly ReadOnlyMemory<byte> SliceMemoryFromHere() => _buffer[_position..];

        /// <summary>
        /// Returns a <see cref="ReadOnlyMemory{T}"/> slice from the specified position to the end of the buffer.
        /// This does not modify the current position.
        /// </summary>
        /// <param name="position">The absolute byte offset within the buffer.</param>
        /// <returns>A <see cref="ReadOnlyMemory{T}"/> over the requested region.</returns>
        public readonly ReadOnlyMemory<byte> SliceMemory(int position) => _buffer[position..];

        /// <summary>
        /// Returns a <see cref="ReadOnlyMemory{T}"/> slice from the specified position with the given length.
        /// This does not modify the current position.
        /// </summary>
        /// <param name="position">The absolute byte offset within the buffer.</param>
        /// <param name="size">The number of bytes in the slice.</param>
        /// <returns>A <see cref="ReadOnlyMemory{T}"/> over the requested region.</returns>
        public readonly ReadOnlyMemory<byte> SliceMemory(int position, int size) => _buffer.Slice(position, size);
        ...
        {
            var result = MemoryMarshal.Read<T>(_buffer.Span[_position..]);
        ...
        /// <param name="position">The absolute byte offset within the buffer to read from.</param>
        /// <returns>The value read from the buffer.</returns>
        public readonly T Read<T>(int position) where T : unmanaged => MemoryMarshal.Read<T>(_buffer.Span[position..]);
        ...
        {
            var result = encoding.GetString(_buffer.Span.Slice(_position, size));
            _position += size;
            return result;
        }
        ...
        public readonly string ReadString(int position, int size, Encoding encoding) =>
            encoding.GetString(_buffer.Span.Slice(position, size));
