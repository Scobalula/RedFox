...
using namespace RedFox::Graphics2D::DirectXTex;
...
RedFox::Graphics2D::DirectXTex::ScratchImage::ScratchImage(IntPtr device, IntPtr context, IntPtr resource)
{
...
RedFox::Graphics2D::DirectXTex::ScratchImage::ScratchImage(Int32 width, Int32 height, ScratchImageFormat format)
{
	InitializeImage(width, height, format);
}

RedFox::Graphics2D::DirectXTex::ScratchImage::ScratchImage(ScratchImageMetadata^ metaData, array<Byte>^ buffer)
{
...
RedFox::Graphics2D::DirectXTex::ScratchImage::ScratchImage(String^ filePath, ScratchImageFileFormat format)
{
	Load(filePath, format);
}

RedFox::Graphics2D::DirectXTex::ScratchImage::ScratchImage(array<Byte>^ buffer, ScratchImageFileFormat format)
{
	Load(buffer, format);
}

void RedFox::Graphics2D::DirectXTex::ScratchImage::InitializeImage(Int32 width, Int32 height, ScratchImageFormat format)
{
	std::unique_ptr<DirectX::ScratchImage> scratchImage(new (std::nothrow) DirectX::ScratchImage);
	if (!scratchImage)
		throw gcnew Exception("Failed to create scratch image");
	...
	DirectX::TexMetadata nativeMetadata =
	{
		...
		0,
		(DXGI_FORMAT)format,
		DirectX::TEX_DIMENSION::TEX_DIMENSION_TEXTURE2D
	};
...
void RedFox::Graphics2D::DirectXTex::ScratchImage::Load(array<Byte>^ buffer, ScratchImageFileFormat format)
{
	HRESULT result;
...
void ScratchImage::Load(String^ filePath, ScratchImageFileFormat format)
{
	...
	case ScratchImageFileFormat::EXR:
	{
		result = DirectX::LoadFromEXRFile(filePathStd.data(), nullptr, *scratchImage);
		break;
	}
	}
...
void ScratchImage::Save(String^ filePath, ScratchImageFileFormat format)
{
	...
	case ScratchImageFileFormat::TGA:
		...
		break;
	case ScratchImageFileFormat::EXR:
		result = DirectX::SaveToEXRFile(*images, filePathStd.data());
		break;
	}
...
void RedFox::Graphics2D::DirectXTex::ScratchImage::Save(System::IO::Stream^ stream, ScratchImageFileFormat format)
{
	if (!ScratchImagePointer)
		throw gcnew ScratchImageException(String::Format("Failed to aquire DirectX::ScratchImage, result returned nullptr"));
...
void RedFox::Graphics2D::DirectXTex::ScratchImage::GenerateMipMaps(int mipMapCount)
{
	if (!ScratchImagePointer)
		throw gcnew ScratchImageException(String::Format("Failed to aquire DirectX::ScratchImage, result returned nullptr"));
...
IntPtr RedFox::Graphics2D::DirectXTex::ScratchImage::GetPixelsPointer(Int64% size)
{
	return GetPixelsPointer(0, 0, 0, size);
}

IntPtr RedFox::Graphics2D::DirectXTex::ScratchImage::GetPixelsPointer(int mip, int item, int slice, Int64% size)
{
...
{
	auto image = ScratchImagePointer->GetImage(mip, item, slice);
	auto imagePixels = image->pixels;

	if (!imagePixels)
		return nullptr;

	auto buffer = gcnew array<Byte>((int)image->slicePitch);

	Marshal::Copy(IntPtr(imagePixels), buffer, 0, (int)image->slicePitch);

	return buffer;
}

Vector4 RedFox::Graphics2D::DirectXTex::ScratchImage::GetPixelValue(int mip, int item, int slice, int x, int y)
{
...
void RedFox::Graphics2D::DirectXTex::ScratchImage::GetPixelValue(int mip, int item, int slice, int x, int y, Vector4% value)
{
	if (x > Width)
	...
	case ScratchImageFormat::R16UNorm:
	{
	...
	case ScratchImageFormat::B8G8R8A8UNorm:
	{
		pixelIndex = ((y * Width) + x) * 4;
		const uint8_t* pixels = img->pixels;
		value.X = pixels[pixelIndex + 2] / 255.0f;
		value.Y = pixels[pixelIndex + 1] / 255.0f;
		value.Z = pixels[pixelIndex + 0] / 255.0f;
		value.W = pixels[pixelIndex + 3] / 255.0f;
		break;
	}
	default:
		throw gcnew Exception("Unsupported Image Format for GetPixelValue(...)");
	}
}

void RedFox::Graphics2D::DirectXTex::ScratchImage::SetPixelValue(int mip, int item, int slice, int x, int y, Vector4 value)
{
	if (x > Width)
		throw gcnew ArgumentOutOfRangeException("x", "X must be less than the Width of the Image");
	if (y > Height)
	...
	case ScratchImageFormat::B8G8R8A8UNorm:
	{
		pixelIndex = ((y * Width) + x) * 4;
		uint8_t* pixels = img->pixels;
		pixels[pixelIndex + 2] = (uint8_t)(Math::Clamp(value.X, 0.0f, 1.0f) * 255.0f);
		pixels[pixelIndex + 1] = (uint8_t)(Math::Clamp(value.Y, 0.0f, 1.0f) * 255.0f);
		pixels[pixelIndex + 0] = (uint8_t)(Math::Clamp(value.Z, 0.0f, 1.0f) * 255.0f);
		pixels[pixelIndex + 3] = (uint8_t)(Math::Clamp(value.W, 0.0f, 1.0f) * 255.0f);
		break;
	}
	default:
		throw gcnew Exception("Unsupported Image Format for SetPixelValue(...)");
	}
}

IntPtr RedFox::Graphics2D::DirectXTex::ScratchImage::CreateShaderResourceView(IntPtr device)
{
