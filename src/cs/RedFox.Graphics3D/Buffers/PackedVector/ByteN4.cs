...
{
    /// <summary>
    /// Packs a 4-component signed normalized byte vector into a single <see cref="uint"/>.
    /// </summary>
    public struct ByteN4 : IPackedVector<ByteN4>
    {
        ...
        public ByteN4(uint packedValue) => _packed = packedValue;
        ...
        public ByteN4(float x, float y, float z, float w) => Pack(new Vector4(x, y, z, w));
        ...
        public void Pack(Vector4 value)
        {
            ...
            var z = (sbyte)Math.Clamp(MathF.Truncate(Math.Clamp(value.Z, -1f, 1f) * 127f), -128f, 127f);
            var w = (sbyte)Math.Clamp(MathF.Truncate(Math.Clamp(value.W, -1f, 1f) * 127f), -128f, 127f);
