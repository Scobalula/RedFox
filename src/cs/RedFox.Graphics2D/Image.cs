...
public sealed class Image
{
    ...
    /// <summary>
    /// Reinterprets the image as a different format without converting pixel data.
    /// Only succeeds when both formats have the same physical size per pixel (uncompressed)
    /// or the same block size (block-compressed). For example, BC3Unorm → BC3UnormSrgb is
    /// valid, but BC3Unorm → R8G8B8A8Unorm is not (use <see cref="Convert"/> instead).
    /// </summary>
    /// <param name="targetFormat">The format to reinterpret the pixel data as.</param>
    /// <exception cref="ArgumentException">
    /// Thrown when the target format has a different physical size than the current format.
    /// </exception>
    public void SetFormat(ImageFormat targetFormat)
    {
        if (Format == targetFormat)
            return;

        var currentIsBC = ImageFormatInfo.IsBlockCompressed(Format);
        var targetIsBC = ImageFormatInfo.IsBlockCompressed(targetFormat);

        if (currentIsBC != targetIsBC)
            throw new ArgumentException($"Cannot reinterpret between block-compressed ({Format}) and uncompressed ({targetFormat}) formats. Use Convert() instead.", nameof(targetFormat));

        if (currentIsBC)
        {
            if (ImageFormatInfo.GetBlockSize(Format) != ImageFormatInfo.GetBlockSize(targetFormat))
                throw new ArgumentException($"Block size mismatch: {Format} ({ImageFormatInfo.GetBlockSize(Format)} bytes) vs {targetFormat} ({ImageFormatInfo.GetBlockSize(targetFormat)} bytes).", nameof(targetFormat));
        }
        else
        {
            if (ImageFormatInfo.GetBitsPerPixel(Format) != ImageFormatInfo.GetBitsPerPixel(targetFormat))
                throw new ArgumentException($"Bits-per-pixel mismatch: {Format} ({ImageFormatInfo.GetBitsPerPixel(Format)} bpp) vs {targetFormat} ({ImageFormatInfo.GetBitsPerPixel(targetFormat)} bpp). Use Convert() instead.", nameof(targetFormat));
        }
    ...
    /// <summary>
    /// Gets the total size in bytes of the pixel buffer.
    /// </summary>
    public int GetPixelDataSize() => _pixels.Length;

    /// <summary>
    /// Gets the row pitch and slice pitch for the top-level (mip 0) image.
    /// </summary>
    /// <returns>A tuple containing the row pitch and slice pitch in bytes.</returns>
    public (int RowPitch, int SlicePitch) GetPitch() => ImageFormatInfo.CalculatePitch(Format, Width, Height);
