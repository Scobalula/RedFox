...
""";
    ...
    public void BvhTranslator_Write_StaticSkeleton_WritesSingleFrame()
    {
        ...
        SkeletonBone[] loadedBones = loaded.FirstOfType<Skeleton>().GetBones();
        Assert.Equal(2, loadedBones.Length);
    }
    ...
    private static Scene CreateAnimatedBvhScene()
    {
        ...
        Skeleton skeleton = scene.RootNode.AddNode<Skeleton>("Skeleton");
        ...
        SkeletonAnimation animation = scene.RootNode.AddNode(new SkeletonAnimation("Walk", skeleton, 2, TransformType.Absolute)
        {
            Framerate = 30.0f,
    ...
    private static Scene CreateWrapBoundaryScene()
    {
        ...
        SkeletonAnimation animation = scene.RootNode.AddNode(new SkeletonAnimation("Wrap", skeleton, 1, TransformType.Absolute)
        {
            Framerate = 30.0f,
            TransformSpace = TransformSpace.Local,
            TransformType = TransformType.Absolute,
